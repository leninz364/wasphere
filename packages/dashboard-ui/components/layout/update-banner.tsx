    ...
    <>
      <div className="flex items-center gap-3 border-b border-primary/20 bg-primary/5 px-4 py-2 text-sm">
        ...
        <span className="text-foreground">
          Actualización disponible — <span className="font-semibold">v{info.latest}</span>{" "}
          <span className="text-muted-foreground">(estás en v{info.current})</span>
        </span>
        <Button type="button" variant="link" size="sm" className="h-auto p-0 text-primary" onClick={() => setOpen(true)}>
          Novedades y cómo actualizar
        </Button>
        <button type="button" onClick={dismiss} aria-label="Descartar" className="ml-auto text-muted-foreground hover:text-foreground">
          <X className="size-4" />
        </button>
      </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent showCloseButton className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Actualizar a v{info.latest}</DialogTitle>
            <DialogDescription>Estás ejecutando v{info.current}.</DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-col gap-1.5">
            <span className="text-sm font-medium text-foreground">Cómo actualizar (Docker)</span>
            <pre className="rounded-md bg-zinc-900 text-zinc-100 p-3 text-xs overflow-x-auto">{`cd /path/to/wasphere
git pull
docker compose pull
docker compose up -d --build`}</pre>
            <p className="text-xs text-muted-foreground">
              Descarga el código/imágenes más recientes y recrea los contenedores. Tus volúmenes de datos se conservan.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 pt-1">
            {info.url && (
              <a href={info.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-sm text-primary hover:underline">
                Ver la versión en GitHub <ExternalLink className="size-3.5" />
              </a>
            )}
          </div>
        </DialogContent>
