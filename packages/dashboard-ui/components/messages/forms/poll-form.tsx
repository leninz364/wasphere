...
export function PollForm({ onSubmit, submitting }: FormProps) {
  ...
  const fillSample = () => {
    setName("¿Qué función de WhatsApp usas más?")
    setOptions(["Mensajes", "Llamadas de voz", "Videollamadas", "Estados"])
    setErrors({})
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim()) errs.name = "La pregunta de la encuesta es obligatoria."
    const filledOptions = options.filter((o) => o.trim())
    if (filledOptions.length < 2) errs.options = "Se requieren al menos 2 opciones."
    const count = parseInt(selectableCount, 10)
    if (isNaN(count) || count < 1 || count > 12) {
      errs.selectableCount = "La cantidad seleccionable debe estar entre 1 y 12."
    }
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1.5">
        <div className="flex items-center justify-between">
          <Label htmlFor="poll-name">Pregunta de la encuesta</Label>
      ...
      <div className="flex flex-col gap-2">
        <Label>Opciones ({options.length}/12)</Label>
        {errors.options && (
      ...
      <div className="flex flex-col gap-1.5">
        <Label htmlFor="poll-count">Cantidad seleccionable</Label>
        <Input
          id="poll-count"
          type="number"
          min={1}
        ...
        <p className="text-xs text-muted-foreground">
          Cuántas opciones puede seleccionar un destinatario
        </p>
      </div>
